 ...
 * @param {ThreadChannel|null} opts.existingThread - existing forum post to migrate into (optional)
...
export async function migrateChannel(opts) {
  const {
    sourceChannel, forumChannel, existingThread, postName, tagName,
  ...
  if (!existingThread && forumChannel.type !== ChannelType.GuildForum) {
  ...
  const targetPerms = existingThread
    ? existingThread.permissionsFor(me)
    : forumChannel.permissionsFor(me);
  ...
  if (!targetPerms.has('SendMessages'))    missing.push('Send Messages (target)');
  if (!targetPerms.has('ManageWebhooks'))  missing.push('Manage Webhooks (target)');
  if (!existingThread && !targetPerms.has('CreatePublicThreads')) {
    missing.push('Create Posts (forum)');
  }
  if (missing.length > 0) {
  ...
  // ── Use existing thread or create new forum post ───────────────────
  let thread;
  if (existingThread) {
    // Use existing forum post
    thread = existingThread;

    // Send a migration header to the existing thread
    const headerContent = [
      `## 📥 Migrating messages from #${sourceChannel.name}`,
      '',
      ...
      '',
      '*Messages below are from the original channel, preserved with their original authors and timestamps.*',
      '',
      '───────────────────────',
    ].filter(Boolean).join('\n');

    try {
      await thread.send(headerContent);
    } catch (err) {
      throw new Error(`Failed to send to existing thread: ${err.message}`);
    }
  } else {
